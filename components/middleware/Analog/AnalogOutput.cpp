...
int AnalogOutputAD5413::init(void)
{
    int ret = 0;
    ...
    if (!_isInitialized) {
        ...
        ret |= ad5413_init(&_device, _config);

        /* Perform commands to start the device */
        ret |= ad5413_soft_reset(_device);
        ret |= ad5413_calib_mem_refresh(_device);
        ret |= ad5413_clear_dig_diag_flag(_device, DIG_DIAG_RESET_OCCURRED);
...
int AnalogOutputAD5413::setMode(AnalogOutput_Mode_t mode)
{
    ...
    /* Set mode sequence */
    ret |= ad5413_dac_out_en(_device, 0);
    ret |= ad5413_dac_input_write(_device, 0);
    ret |= ad5413_soft_ldac_cmd(_device);
    ret |= ad5413_internal_buffers_en(_device, 1);
    if (_mode == AOUT_MODE_M10V5_10V5) {
        ret |= ad5758_set_output_range(_device, OUTPUT_RANGE_M10V5_10V5);
    } else if (_mode == AOUT_MODE_0mA_20mA) {
        ret |= ad5758_set_output_range(_device, OUTPUT_RANGE_0mA_20mA);
    } else {
        ESP_LOGE(TAG, "Undefined mode");
        ret = -1;
    }    
    ret |= ad5413_dac_out_en(_device, 1);
...
int AnalogOutputAD5413::write(float value)
{
    ...
    ret |= ad5413_dac_input_write(_device, data);
    if (ret == -1) {
