...
int DigitalOutputs::init(const gpio_num_t *gpio, const adc_num_t *adc, int nb)
{
    ...
    for (uint8_t i = 0; i < _nb; i++) {
        if (_adc_current[i].unit == ADC_UNIT_1) {
            err |=
                adc1_config_channel_atten((adc1_channel_t)_adc_current[i].channel, ADC_ATTEN_DB_11);
        } else if (_adc_current[i].unit == ADC_UNIT_2) {
            err |=
                adc2_config_channel_atten((adc2_channel_t)_adc_current[i].channel, ADC_ATTEN_DB_11);
        } else {
            ESP_LOGE(TAG, "Invalid ADC channel");
        }
    }

    esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_DB_11, (adc_bits_width_t)ADC_WIDTH_BIT_DEFAULT,
                             1100, &_adc1Characteristics);
    for (uint8_t i = 0; i < _nb; i++) {
        if (_adc_current[i].unit == ADC_UNIT_2) {
            esp_adc_cal_characterize(ADC_UNIT_2, ADC_ATTEN_DB_11,
                                     (adc_bits_width_t)ADC_WIDTH_BIT_DEFAULT, 1100,
                                     &_adc2Characteristics);
            break;
        }
    }
    ...
    if (num < _nb) {
        ...
        for (int i = 0; i < DOUT_SENSOR_ADC_NO_OF_SAMPLES; i++) {
            if (_adc_current[num].unit == ADC_UNIT_1) {
                ...
                adc2_get_raw((adc2_channel_t)_adc_current[num].channel,
                             (adc_bits_width_t)ADC_WIDTH_BIT_DEFAULT, &current_reading);
            } else {
                current_reading = 0;
                ESP_LOGE(TAG, "Invalid ADC channel");
            }
        ...
        if (_adc_current[num].unit == ADC_UNIT_1) {
            voltage =
                static_cast<float>(esp_adc_cal_raw_to_voltage(adc_reading, &_adc1Characteristics));
        } else if (_adc_current[num].unit == ADC_UNIT_2) {
            voltage =
                static_cast<float>(esp_adc_cal_raw_to_voltage(adc_reading, &_adc2Characteristics));
        } else {
            ESP_LOGE(TAG, "Invalid ADC channel");
        }
