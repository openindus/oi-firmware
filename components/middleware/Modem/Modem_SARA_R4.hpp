 ...
 */
class SARA_R4: public GenericModule {
    /**
...
{
    return dce_factory::Factory::create_unique_dce_from<SARA_R4, DCE *>(dce_config, std::move(dte), netif);
}
...
extern "C" esp_err_t esp_modem_get_time(esp_modem_dce_t *dce_wrap, char *p_time)
{
    ...
    auto ret = command_response_to_esp_err(static_cast<SARA_R4 *>(dce_wrap->dce->get_module())->get_time(time));
