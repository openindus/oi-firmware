...
static int pingCmd(int argc, char **argv)
{
    ...
    if (Master::ping(type, sn)) {
        ...
        printf("Ping module: %lu time: %lld us\n", sn, (t-t0));
    } else {
        printf("Cannot ping module: %lu\n", sn);
        return 2;
    }
    return 0;
}
...
static int getSlaveIdCmd(int argc, char **argv)
{
    ...
    if (id != 0) {
        ...
        printf("No slave found for board type %u with SN %lu\n", type, sn);
        return 2;
    }
...
static int discoverSlavesCmd(int argc, char **argv) 
{
    ...
    while (it != _ids.end()) {
        printf("{\"id\":%i,\"type\":%i,\"sn\":%lu}", it->first, it->second.first, it->second.second);
        ++it;
