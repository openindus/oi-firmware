...
static void* _temp_sensor = nullptr;
...
int Board::init(void)
{
    ...
    if (temperature_sensor_install(&temp_sensor_config, &temp_sensor) != ESP_OK) {
        ESP_LOGE(TAG, "temperature_sensor_install failed");
        err |= -1;
...
float Board::getTemperature(void)
{
    ...
    if (_temp_sensor != nullptr) {
        temperature_sensor_handle_t temp_sensor = (temperature_sensor_handle_t)_temp_sensor;
        if (temperature_sensor_get_celsius(temp_sensor, &tsens_out) != ESP_OK) {
            ESP_LOGE(TAG, "temperature_sensor_get_celsius failed");
        }
    }
    return tsens_out;
}
