...
)
{
    ...
    cnf_3.sof = true;
    cnf_3.wakfil = false;
    mcp25625_hw_ctl_set(&cnf_1);
    mcp25625_hw_ctl_set(&cnf_2);
    mcp25625_hw_ctl_set(&cnf_3);
...
int mcp2515_getNextFreeTXBuf(TXB_t *txb)                 /* get Next free txbuf          */
{
    TXB_t ctrlregs[3] = { TXB0, TXB1, TXB2 };
    *txb= TXB0;
    mcp25625_txb_ctl tx_res;
    tx_res.reg = CTL_TXB;
                                                                        /* check all 3 TX-Buffers       */
    for (int i=0; i<3; i++) {
        tx_res.buffer=ctrlregs[i];
        mcp25625_hw_ctl_get( &tx_res );
        if ( tx_res.txreq == 0 ) {
            *txb = ctrlregs[i];                                   /* return SIDH-address of Buffer*/
...
)
{
    ...
    do {
        res = mcp2515_getNextFreeTXBuf(&tx_buff);                     
        timeout++;
    } while (res == MCP25625_CTL_ERR && (timeout < TIMEOUTVALUE));
...
{
    mcp25625_hw_ctl_get( ( void* )&tx_ctl );
    tx_ctl.buffer = tx_buff;
    tx_ctl.txreq = true;
    tx_ctl.mask = TXB_TXREQ;
