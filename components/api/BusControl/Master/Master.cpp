...
BusControl_State_t Master::_state = STATE_IDLE;
...
bool Master::autoId(void)
{
    ...
    for (int i=0; i<Command::_instances.size(); i++) {
        if (Command::_instances[i]->getSN() == 0) {
            num_id_auto++;
        } else {
            num_id_sn++;
        }
    ...
    if (num_id_auto) {
        ...
        if (Command::_instances.size() == _ids.size()) {
            std::map<uint16_t, std::pair<uint16_t, uint32_t>>::iterator it = _ids.begin();
            for (int i=0; i<Command::_instances.size(); i++) {
                // First, check if board type is good
                if (it->second.first == Command::_instances[i]->getType()) {
                    // If OK, set ID and SN
                    Command::_instances[i]->setId(it->first);
                    Command::_instances[i]->setSN(it->second.second);
                    ++it;
                    Command::_instances[i]->ledOn(LED_YELLOW);
                    ...
                    ESP_LOGE(TAG, "Type of module %i is incorrect: you have instantiate an %s and module detected is an %s", \
                                        i+1, \
                                        ModuleUtils::typeToName(Command::_instances[i]->getType(), name1), \
            ...
            ESP_LOGE(TAG, "Number of instantiated modules: %d",  Command::_instances.size());
    ...
    /* Initialize module with SN */
        ...
        for (int i=0; i<Command::_instances.size(); i++) {
            uint16_t current_id = Master::_getIdFromSerialNumAndType(Command::_instances[i]->getType(), Command::_instances[i]->getSN());
            if (current_id != 0) {
                Command::_instances[i]->setId(current_id);
                Command::_instances[i]->ledOn(LED_YELLOW);
                vTaskDelay(50/portTICK_PERIOD_MS);
            } else {
                ESP_LOGE(TAG, "Cannot instantiate module with SN:%i",  Command::_instances[i]->getSN());
                return false;
            }
        }
    }
    ...
    for (int i=0; i<Command::_instances.size(); i++) {
        Command::_instances[i]->ledBlink(LED_GREEN, 1000);
    }
    return true;
}
...
void Master::_busTask(void *pvParameters) 
{
    ...
    while (1) {
        if (BusCAN::read(&frame, &id, &size) != -1) { 
            switch (frame.cmd)
            {
                case CMD_EVENT:
                {
                    auto it = Command::_eventCallbacks.find(std::make_pair(frame.args[0], id));
                    if (it != Command::_eventCallbacks.end()) {
                        if (it->second != NULL) {
