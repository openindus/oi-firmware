...
uint16_t Slave::_id;
BusControl_State_t Slave::_state = STATE_IDLE;
TaskHandle_t Slave::_taskHandle = NULL;
std::map<uint8_t, std::function<void(std::vector<uint8_t>&)>> Slave::_ctrlCallbacks;
...
void Slave::_busTask(void *pvParameters) 
{
    BusRS::Frame_t frame;
    frame.length = 1024;
