 ...
 * @brief Master Controller class implementation
...
static const char TAG[] = "MasterController";

Controller_State_t MasterController::_state = STATE_IDLE;
#ifndef LINUX_ARM
TaskHandle_t MasterController::_taskHandle = NULL;
#endif
std::map<uint16_t, std::pair<uint16_t, uint32_t>, std::greater<uint16_t>> MasterController::_ids;
std::map<std::pair<uint8_t, uint16_t>, std::function<void(uint8_t*)>> MasterController::_eventCallbacks;
std::vector<Controller*> MasterController::_instances;

int MasterController::init(void)
{
    int err = 0;
...
void MasterController::start(void)
{
#ifndef LINUX_ARM
    if (_taskHandle != NULL) {
...
void MasterController::stop(void)
{
#ifndef LINUX_ARM
    if (_taskHandle != NULL) {
...
int MasterController::getStatus(void)
{
    return (int)_state;
}

bool MasterController::autoId(void)
{
#ifndef LINUX_ARM
    ...
    /* Initialize module with SN */
        ...
        for (int i=0; i<_instances.size(); i++) {
            uint16_t current_id = MasterController::_getIdFromSerialNumAndType(_instances[i]->getType(), _instances[i]->getSN());
            if (current_id != 0) {
...
void MasterController::program(uint16_t type, uint32_t sn)
{
#ifndef LINUX_ARM
...
bool MasterController::ping(uint16_t type, uint32_t sn) 
{
    BusRS::Frame_t frame;
    frame.cmd = CMD_PING;
    frame.id = 0;
...
void MasterController::getBoardInfo(uint16_t type, uint32_t sn, Board_Info_t* board_info)
{
    uint16_t id = _getIdFromSerialNumAndType(type, sn);
...
std::map<uint16_t,std::pair<uint16_t, uint32_t>,std::greater<uint16_t>> MasterController::discoverSlaves(void)
{
...
uint16_t MasterController::_getIdFromSerialNumAndType(uint16_t type, uint32_t sn)
{
    uint16_t id = 0;
...
void MasterController::_busTask(void *pvParameters) 
{
...
void MasterController::_programmingTask(void *pvParameters)
{
#ifndef LINUX_ARM
