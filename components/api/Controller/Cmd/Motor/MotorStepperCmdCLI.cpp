...
static int advancedParamFunc(int argc, char **argv)
{
    ...
    if (controller == nullptr) {
        ESP_LOGE(TAG, "Failed to create Controller instance");
        return 1;
    }

    MotorStepperCmd *stepper = new MotorStepperCmd(controller);
    if (stepper == nullptr) {
...
{
    advancedParamArgs.id    = arg_int1("i", "id", "<id>", "Controller ID");
