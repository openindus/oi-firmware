...
public:
    ...
    static int init(void) {
        ...
        Slave::addRequestCallback(REQUEST_SET_OVERCURRENT_THRESHOLD, [](std::vector<uint8_t> &data) {
        ...
        Slave::addRequestCallback(REQUEST_ATTACH_OVERCURRENT_CALLBACK, [](std::vector<uint8_t> &data) {
            DigitalOutputs::attachOvercurrentCallback(_overcurrentCallback);
            data.clear();
        });

        Slave::addRequestCallback(REQUEST_DETACH_OVERCURRENT_CALLBACK, [](std::vector<uint8_t> &data) {
