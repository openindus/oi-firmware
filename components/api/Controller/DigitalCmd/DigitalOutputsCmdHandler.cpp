...
{
    Slave::addRequestProcessCallback(REQUEST_DIGITAL_WRITE, [](std::vector<uint8_t> &data) {
        DigitalOutputs::digitalWrite((DOut_Num_t)data[1], (bool)data[2]);
        data.clear();
    });

    Slave::addRequestProcessCallback(REQUEST_TOGGLE_OUTPUT, [](std::vector<uint8_t> &data) {
        DigitalOutputs::toggleOutput((DOut_Num_t)data[1]);
        data.clear();
    });

    Slave::addRequestProcessCallback(REQUEST_OUTPUT_MODE, [](std::vector<uint8_t> &data) {
        DigitalOutputs::outputMode((DOut_Num_t)data[1], (DOut_Mode_t)data[2]);
        data.clear();
    });

    Slave::addRequestProcessCallback(REQUEST_SET_PWM_FREQUENCY, [](std::vector<uint8_t> &data) {
    ...
    Slave::addRequestProcessCallback(REQUEST_GET_OUTPUT_CURRENT, [](std::vector<uint8_t> &data) {
