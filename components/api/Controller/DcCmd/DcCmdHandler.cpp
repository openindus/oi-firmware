...
int DcCmdHandler::init() {
    ...
    Slave::addRequestProcessCallback(REQUEST_MOTOR_DC_RUN, [](std::vector<uint8_t>& data) {
    ...
    Slave::addRequestProcessCallback(REQUEST_MOTOR_DC_STOP, [](std::vector<uint8_t>& data) {
        MotorNum_t motor = static_cast<MotorNum_t>(data[1]);
        MotorDc::stop(motor);
        data.clear();
    });
