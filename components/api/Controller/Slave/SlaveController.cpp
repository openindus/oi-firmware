 ...
 * @brief Slave Controller class implementation
...
static const char TAG[] = "SlaveController";

uint16_t SlaveController::_id;
Controller_State_t SlaveController::_state = STATE_IDLE;
TaskHandle_t SlaveController::_taskHandle = NULL;
std::map<uint8_t, std::function<void(std::vector<uint8_t>&)>> SlaveController::_handlers;

int SlaveController::init(void)
{
    int err = 0;
...
void SlaveController::start(void)
{
    if (_taskHandle != NULL) {
...
void SlaveController::stop(void)
{
    if (_taskHandle != NULL) {
...
int SlaveController::getStatus(void)
{
    return (int)_state;
}
 ...
 * @param msgBytes 
 */
void SlaveController::sendEvent(std::vector<uint8_t> msgBytes)
{
...
void SlaveController::_busTask(void *pvParameters) 
{
    BusRS::Frame_t frame;
    frame.length = 1024;
...
void SlaveController::_heartbeatTask(void *pvParameters)
{
