...
GenericSensorCmd::GenericSensorCmd(ModuleControl* module, uint8_t index, Sensor_Type_e type, std::array<AIn_Num_t, 4> ain_pins) :
_module(module),
_index(index),
_type(type),
_ain_pins(ain_pins)
{
    /* Create the queue for wait function and add callbackId for CAN event */
    _readEvent = xQueueCreate(1, sizeof(uint8_t*));
}

int16_t GenericSensorCmd::getInt16(uint8_t eventId, uint8_t callbackId)
{
    // Add eventId (remove eventId if it already exists)
    Master::removeEventCallback(eventId, _module->getId());
    Master::addEventCallback(eventId, _module->getId(), [this](uint8_t* data) {
        xQueueSend(_readEvent, &data, pdMS_TO_TICKS(100));
    });

    // Send a message to slave to request a read but do not wait a response
    std::vector<uint8_t> msgBytes = {(uint8_t)callbackId, _index};
...
float GenericSensorCmd::getFloat(uint8_t eventId, uint8_t callbackId)
{
    // Add callbackId (remove callbackId if it already exists)
    Master::removeEventCallback(eventId, _module->getId());
    Master::addEventCallback(eventId, _module->getId(), [this](uint8_t* data) {
        xQueueSend(_readEvent, &data, pdMS_TO_TICKS(100));
    });

    // Send a message to slave to callbackId a read but do not wait a response
    std::vector<uint8_t> msgBytes = {(uint8_t)callbackId, _index};
    _module->runCallback(msgBytes, false);
...
{
    return getInt16(EVENT_SENSOR_VALUE_RAW, CALLBACK_SENSOR_READ_RAW);
}

float GenericSensorCmd::read(void)
{
    return getFloat(EVENT_SENSOR_VALUE, CALLBACK_SENSOR_READ);
}

void GenericSensorCmd::setParameter(Sensor_Parameter_e parameter, Sensor_Parameter_Value_u value)
{
    std::vector<uint8_t> msgBytes = {CALLBACK_SENSOR_SET_PARAMETER, _index, *((uint8_t *) &parameter)};
...
{
    return getFloat(EVENT_SENSOR_VALUE_MILLIVOLT, CALLBACK_SENSOR_READ_MILLIVOLT);
}
...
{
    return getFloat(EVENT_SENSOR_VALUE_RESISTANCE, CALLBACK_SENSOR_READ_RESISTANCE);
}

float RTDCmd::readTemperature(void)
{
    return getFloat(EVENT_SENSOR_VALUE_TEMPERATURE, CALLBACK_SENSOR_READ_TEMPERATURE);
}
...
int AnalogInputsLSCmd::addSensor(Sensor_Type_e type, const std::vector<AIn_Num_t>& aIns)
{
    ...
    if (index >= 0)
    {
        ...
        case RAW_SENSOR:
            ...
            sensor_ptr = new RawSensorCmd(_module, index, type, aInsArray);
        ...
        case RTD_PT1000:
            ...
            sensor_ptr = new RTDCmd(_module, index, type, aInsArray);
            sensors.emplace_back(sensor_ptr);
            return index;
        case THERMOCOUPLE_B:
        case THERMOCOUPLE_E:
        ...
        case THERMOCOUPLE_T:
            ...
            sensor_ptr = new ThermocoupleCmd(_module, index, type, aInsArray);
            sensors.emplace_back(sensor_ptr);
            return index;
        case STRAIN_GAUGE:
            if (nbr_pin_set < 4) {
            ...
            sensor_ptr = new StrainGaugeCmd(_module, index, type, aInsArray);
            sensors.emplace_back(sensor_ptr);
            return index;
        default:
