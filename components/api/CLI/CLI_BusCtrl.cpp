...
static int programCmd(int argc, char **argv)
{
    ...
    BusCtrlMaster::program(type, sn);

    return 0;
}

int CLI_BusCtrl::_registerProgramCmd(void)
{
...
int CLI_BusCtrl::_registerPingCmd(void)
{
...
{
    BusCtrlMaster::autoId();
    return 0;
}

int CLI_BusCtrl::_registerAutoIdCmd(void)
{
    const esp_console_cmd_t cmd = {
        .command = "auto-id",
...
static int discoverSlavesCmd(int argc, char **argv) 
{
    ...
    std::map<uint16_t,std::pair<uint16_t, uint32_t>,std::greater<uint16_t>> _ids = BusCtrlMaster::discoverSlaves();
...
int CLI_BusCtrl::_registerDiscoverSlavesCmd(void)
{
    const esp_console_cmd_t cmd = {
...
static int getSlaveInfoCmd(int argc, char **argv)
{
    ...
    if (getSlaveInfoArgs.type->count == 1 && getSlaveInfoArgs.sn->count == 1) {
        BusCtrlMaster::getBoardInfo((uint16_t)getSlaveInfoArgs.type->ival[0], (uint32_t)getSlaveInfoArgs.sn->ival[0], &boardInfo);
    } else {
        return -1;
    }
...
int CLI_BusCtrl::_registerGetSlaveInfoCmd(void)
{
...
int CLI_BusCtrl::_registerStopCmd(void)
{
    const esp_console_cmd_t cmd = {
        .command = "stop",
...
int CLI_BusCtrl::_registerStartCmd(void)
{
    const esp_console_cmd_t cmd = {
        .command = "start",
...
#if defined(MODULE_MASTER)
    state = BusCtrlMaster::getStatus();
#elif defined(MODULE_SLAVE)
    state = BusCtrlSlave::getStatus();
#else 
...
int CLI_BusCtrl::_registerGetStatusCmd(void)
{
    const esp_console_cmd_t cmd = {
        .command = "status",
