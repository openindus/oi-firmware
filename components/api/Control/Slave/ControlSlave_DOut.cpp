...
int ControlSlave_DOut::init() {
    ...
    ControlSlave::addCtrlCallback(REQUEST_TOGGLE_OUTPUT, [](std::vector<uint8_t>& data) {
    ...
    ControlSlave::addCtrlCallback(REQUEST_SET_PWM_FREQUENCY, [](std::vector<uint8_t>& data) {
        uint32_t* freq = reinterpret_cast<uint32_t*>(&data[2]);
        DigitalOutputs::setPWMFrequency((DOut_Num_t)data[1], *freq);
        data.clear();
    });

    ControlSlave::addCtrlCallback(REQUEST_SET_PWM_DUTY_CYCLE, [](std::vector<uint8_t>& data) {
        uint32_t* duty = reinterpret_cast<uint32_t*>(&data[2]);
        DigitalOutputs::setPWMDutyCycle((DOut_Num_t)data[1], *duty);
        data.clear();
    });

    ControlSlave::addCtrlCallback(REQUEST_GET_OUTPUT_CURRENT, [](std::vector<uint8_t>& data) {
