...
void BrushlessSlave::init(void)
{
    ...
    onRequest(MOTOR_SET_BRAKE, [](RequestMsg_t msg) -> uint32_t {
        bool brake = (bool)msg.data;
        BrushlessStandalone::setBrake(brake);
        return 0;
    });

    onRequest(MOTOR_SET_DIRECTION, [](RequestMsg_t msg) -> uint32_t {
    ...
    onRequest(ENCODER_GET_SPEED, [](RequestMsg_t msg) -> uint32_t {
    ...
    onRequest(ENCODER_GET_DIRECTION, [](RequestMsg_t msg) -> uint32_t {
        return BrushlessStandalone::getDirection();  
    });

    onRequest(ENCODER_GET_POSITION, [](RequestMsg_t msg) -> uint32_t {
