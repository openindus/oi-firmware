...
int EncoderCmd::begin(DIn_Num_t A, DIn_Num_t B, int16_t ppr)
{
    ...
    // return _module->runCallback(CALLBACK_ENCODER_BEGIN, A, B, ppr);
}

void EncoderCmd::end(void)
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_END, (uint8_t)_instance};
    _module->runCallback(msgBytes);
}

void EncoderCmd::reset(void)
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_RESET, (uint8_t)_instance};
    _module->runCallback(msgBytes);
}

int EncoderCmd::getRevolutions(void)
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_GET_REVOLUTIONS, (uint8_t)_instance};
    _module->runCallback(msgBytes);
...
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_GET_PULSES, (uint8_t)_instance};
    _module->runCallback(msgBytes);
...
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_GET_ANGLE, (uint8_t)_instance};
    _module->runCallback(msgBytes);
...
{
    std::vector<uint8_t> msgBytes = {CALLBACK_ENCODER_GET_SPEED, (uint8_t)_instance};
    _module->runCallback(msgBytes);
