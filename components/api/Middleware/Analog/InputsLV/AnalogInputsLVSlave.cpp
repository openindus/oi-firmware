...
int AnalogInputsLVSlave::init() {
    ...
    ModuleSlave::addCtrlCallback(CONTROL_ANALOG_READ_AMP, [](std::vector<uint8_t>& data) {
        float value = AnalogInputsLV::analogReadAmp((AnalogInput_Num_t)data[1]);
        uint8_t* ptr = reinterpret_cast<uint8_t*>(&value);
        data.insert(data.end(), ptr, ptr + sizeof(float));
    });

    ModuleSlave::addCtrlCallback(CONTROL_ANALOG_READ_MILLIAMP, [](std::vector<uint8_t>& data) {
