...
void BusCtrlCmd_DOut::digitalWrite(DOut_Num_t num, uint8_t level)
{
    std::vector<uint8_t> msgBytes = {REQUEST_DIGITAL_WRITE, (uint8_t)num, (uint8_t)level};
    _control->request(msgBytes);
}

void BusCtrlCmd_DOut::toggleOutput(DOut_Num_t num)
{
    std::vector<uint8_t> msgBytes = {REQUEST_TOGGLE_OUTPUT, (uint8_t)num};
    _control->request(msgBytes);
}

void BusCtrlCmd_DOut::outputMode(DOut_Num_t num, DOut_Mode_t mode)
{
    std::vector<uint8_t> msgBytes = {REQUEST_OUTPUT_MODE, (uint8_t)num, (uint8_t)mode};
    _control->request(msgBytes);
}

void BusCtrlCmd_DOut::setPWMFrequency(DOut_Num_t num, uint32_t freq)
{
...
void BusCtrlCmd_DOut::setPWMDutyCycle(DOut_Num_t num, uint32_t duty)
{
...
float BusCtrlCmd_DOut::getOutputCurrent(DOut_Num_t num)
{
