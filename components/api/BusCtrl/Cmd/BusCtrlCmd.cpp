...
static const char TAG[] = "BusCtrlCmd";

std::map<std::pair<uint8_t, uint16_t>, std::function<void(uint8_t)>> BusCtrlCmd::_eventCallbacks;
std::vector<BusCtrlCmd*> BusCtrlCmd::_instances;
 ...
 * @param color 
 */
void BusCtrlCmd::ledOn(LedColor_t color)
{
 ...
 * @param period 
 */
void BusCtrlCmd::ledBlink(LedColor_t color, uint32_t period)
{
 ...
 */
int BusCtrlCmd::request(std::vector<uint8_t>& msgBytes, bool ackNeeded)
{
    BusRS::Frame_t frame;
 ...
 */
void BusCtrlCmd::_ledStatus(LedState_t state, LedColor_t color, uint32_t period)
{
    uint8_t payload[6];
