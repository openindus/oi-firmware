...
static const char TAG[] = "BusCtrlMaster";

BusCtrl_State_t BusCtrlMaster::_state = STATE_IDLE;
TaskHandle_t BusCtrlMaster::_taskHandle = NULL;
std::map<uint16_t, std::pair<uint16_t, uint32_t>, std::greater<uint16_t>> BusCtrlMaster::_ids;

int BusCtrlMaster::init(void)
{
    int err = 0;
...
int BusCtrlMaster::getStatus(void)
{
    return (int)_state;
}

bool BusCtrlMaster::autoId(void)
{
    ...
    for (int i=0; i<BusCtrlCmd::_instances.size(); i++) {
        if (BusCtrlCmd::_instances[i]->getSN() == 0) {
            num_id_auto++;
        } else {
            num_id_sn++;
        }
    ...
    if (num_id_auto) {
        ...
        if (BusCtrlCmd::_instances.size() == _ids.size()) {
            std::map<uint16_t, std::pair<uint16_t, uint32_t>>::iterator it = _ids.begin();
            for (int i=0; i<BusCtrlCmd::_instances.size(); i++) {
                // First, check if board type is good
                if (it->second.first == BusCtrlCmd::_instances[i]->getType()) {
                    // If OK, set ID and SN
                    BusCtrlCmd::_instances[i]->setId(it->first);
                    BusCtrlCmd::_instances[i]->setSN(it->second.second);
                    ++it;
                    BusCtrlCmd::_instances[i]->ledOn(LED_YELLOW);
                    ...
                    ESP_LOGE(TAG, "Type of module %i is incorrect: you have instantiate an %s and module detected is an %s", \
                                        i+1, \
                                        ModuleUtils::typeToName(BusCtrlCmd::_instances[i]->getType(), name1), \
            ...
            ESP_LOGE(TAG, "Number of instantiated modules: %d",  BusCtrlCmd::_instances.size());
    ...
    /* Initialize module with SN */
        ...
        for (int i=0; i<BusCtrlCmd::_instances.size(); i++) {
            uint16_t current_id = BusCtrlMaster::_getIdFromSerialNumAndType(BusCtrlCmd::_instances[i]->getType(), BusCtrlCmd::_instances[i]->getSN());
            if (current_id != 0) {
                BusCtrlCmd::_instances[i]->setId(current_id);
                BusCtrlCmd::_instances[i]->ledOn(LED_YELLOW);
                vTaskDelay(50/portTICK_PERIOD_MS);
            } else {
                ESP_LOGE(TAG, "Cannot instantiate module with SN:%i",  BusCtrlCmd::_instances[i]->getSN());
                return false;
            }
        }
    }
    ...
    for (int i=0; i<BusCtrlCmd::_instances.size(); i++) {
        BusCtrlCmd::_instances[i]->ledBlink(LED_GREEN, 1000);
    }
    return true;
}
...
void BusCtrlMaster::program(uint16_t type, uint32_t sn)
{
...
bool BusCtrlMaster::ping(uint16_t type, uint32_t sn) 
{
    BusRS::Frame_t frame;
    frame.cmd = CMD_PING;
    frame.id = 0;
...
void BusCtrlMaster::getBoardInfo(uint16_t type, uint32_t sn, Board_Info_t* board_info)
{
    uint16_t id = _getIdFromSerialNumAndType(type, sn);
...
std::map<uint16_t,std::pair<uint16_t, uint32_t>,std::greater<uint16_t>> BusCtrlMaster::discoverSlaves(void)
{
...
uint16_t BusCtrlMaster::_getIdFromSerialNumAndType(uint16_t type, uint32_t sn)
{
    uint16_t id = 0;
...
void BusCtrlMaster::_busTask(void *pvParameters) 
{
    ...
    while (1) {
        if (BusCAN::read(&frame, &id, &size) != -1) { 
            switch (frame.cmd)
            {
                case CMD_EVENT:
                {
                    auto it = BusCtrlCmd::_eventCallbacks.find(std::make_pair(frame.args[0], id));
                    if (it != BusCtrlCmd::_eventCallbacks.end()) {
                        if (it->second != NULL) {
...
void BusCtrlMaster::_programmingTask(void *pvParameters)
{
