...
int BusCtrlSlave_DOut::init() {
    ...
    BusCtrlSlave::addCtrlCallback(REQUEST_OUTPUT_MODE, [](std::vector<uint8_t>& data) {
        DigitalOutputs::outputMode((DOut_Num_t)data[1], (DOut_Mode_t)data[2]);
        data.clear();
    });

    BusCtrlSlave::addCtrlCallback(REQUEST_SET_PWM_FREQUENCY, [](std::vector<uint8_t>& data) {
        uint32_t* freq = reinterpret_cast<uint32_t*>(&data[2]);
        DigitalOutputs::setPWMFrequency((DOut_Num_t)data[1], *freq);
        data.clear();
    });

    BusCtrlSlave::addCtrlCallback(REQUEST_SET_PWM_DUTY_CYCLE, [](std::vector<uint8_t>& data) {
        uint32_t* duty = reinterpret_cast<uint32_t*>(&data[2]);
        DigitalOutputs::setPWMDutyCycle((DOut_Num_t)data[1], *duty);
        data.clear();
    });

    BusCtrlSlave::addCtrlCallback(REQUEST_GET_OUTPUT_CURRENT, [](std::vector<uint8_t>& data) {
