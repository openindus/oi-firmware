...
int BusCtrlSlave_AOut::init() {
    
    BusCtrlSlave::addCtrlCallback(REQUEST_ANALOG_OUTPUT_MODE, [](std::vector<uint8_t>& data) {
        AnalogOutputs::analogOutputMode((AnalogOutput_Num_t)data[1], (AnalogOutput_Mode_t)data[2]);
        data.clear();
    });

    BusCtrlSlave::addCtrlCallback(REQUEST_ANALOG_WRITE, [](std::vector<uint8_t>& data) {
