...
static const char TAG[] = "BusCtrlSlave";

uint16_t BusCtrlSlave::_id;
BusCtrl_State_t BusCtrlSlave::_state = STATE_IDLE;
TaskHandle_t BusCtrlSlave::_taskHandle = NULL;
std::map<uint8_t, std::function<void(std::vector<uint8_t>&)>> BusCtrlSlave::_ctrlCallbacks;

int BusCtrlSlave::init(void)
{
    int err = 0;
...
int BusCtrlSlave::getStatus(void)
{
    return (int)_state;
}
 ...
 * @param msgBytes 
 */
void BusCtrlSlave::sendEvent(std::vector<uint8_t> msgBytes)
{
...
void BusCtrlSlave::_busTask(void *pvParameters) 
{
    BusRS::Frame_t frame;
    frame.length = 1024;
...
void BusCtrlSlave::_heartbeatTask(void *pvParameters)
{
