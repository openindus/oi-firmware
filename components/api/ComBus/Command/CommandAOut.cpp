...
int CommandAOut::analogOutputMode(AnalogOutput_Num_t num, AnalogOutput_Mode_t mode)
{
    std::vector<uint8_t> msgBytes = {REQUEST_ANALOG_OUTPUT_MODE, (uint8_t)num, (uint8_t)mode};
    return _control->request(msgBytes);
}

int CommandAOut::analogWrite(AnalogOutput_Num_t num, float value)
{
