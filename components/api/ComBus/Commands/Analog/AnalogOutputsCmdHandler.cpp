...
int AnalogOutputsCmdHandler::init() {
    ...
    ControllerSlave::addCmdHandler(REQUEST_ANALOG_OUTPUT_MODE, [](std::vector<uint8_t>& data) {
        AnalogOutputs::analogOutputMode((AnalogOutput_Num_t)data[1], (AnalogOutput_Mode_t)data[2]);
        data.clear();
    });

    ControllerSlave::addCmdHandler(REQUEST_ANALOG_WRITE, [](std::vector<uint8_t>& data) {
