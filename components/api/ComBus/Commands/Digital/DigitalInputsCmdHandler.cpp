...
int DigitalInputsCmdHandler::init() {
    ...
    ControllerSlave::addCmdHandler(REQUEST_DIGITAL_READ, [](std::vector<uint8_t>& data) { 
        int level = DigitalInputs::digitalRead((DIn_Num_t)data[1]);
        data.push_back(static_cast<uint8_t>(level));
    });

    ControllerSlave::addCmdHandler(REQUEST_ATTACH_INTERRUPT, [](std::vector<uint8_t>& data) { 
        DigitalInputs::attachInterrupt((DIn_Num_t)data[1], _isrCallback[data[1]], (InterruptMode_t)data[2]);
        data.clear();
    });

    ControllerSlave::addCmdHandler(REQUEST_DETACH_INTERRUPT, [](std::vector<uint8_t>& data) { 
