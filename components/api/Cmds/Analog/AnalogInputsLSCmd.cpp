...
int16_t GenericSensorCmd::getInt16(Cmds_Event_e event, Cmds_Request_e request)
{
    // Add callback (remove callback if it already exists)
    MasterController::removeEventCallback(event, _control->getId());
    MasterController::addEventCallback(event, _control->getId(), [this](uint8_t* data) {
...
float GenericSensorCmd::getFloat(Cmds_Event_e event, Cmds_Request_e request)
{
    // Add callback (remove callback if it already exists)
    MasterController::removeEventCallback(event, _control->getId());
    MasterController::addEventCallback(event, _control->getId(), [this](uint8_t* data) {
